...
public class GetVacancyApplicationsServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String recruiterEntripriseNr = request.getParameter("enterpriseNr");
        ...
        try {
            allRecruiterVacancies = this.vacancyHandler.getAllVacancies(recruiterEntripriseNr);
        ...
        try {
            session.setAttribute("recruiterName",
                    new RecruiterHandler(
                            new DatabaseManager()
                    ).getRecruiter(recruiterEntripriseNr).getEnterpriseName());
        } catch (SQLException ex) {
            session.setAttribute("exceptionTitle", "Database Error");
